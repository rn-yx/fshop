 ...
 */
package com.fshop.seckill.controller;
...
import com.fshop.seckill.model.query.SeckillQuery;
...
import com.fshop.seckill.service.SeckillService;
...
public class SeckillController extends BaseController {
    ...
    public GlobalRet getVerifyCode() {
        return success(seckillService.getVerifyCode(ContextHandler.getCurrentUserId()));
    }
    ...
    public GlobalRet getSeckillPath(@RequestParam String verifyCode, @RequestParam long productId) {
        ...
        if (!seckillService.checkVerifyCode(currentUserId, verifyCode)) {
            return failure(GlobalExCode.VERIFY_CODE_ERROR);
        }
        return success(seckillService.getSeckillPath(currentUserId, productId));
    }
    ...
    public GlobalRet<SeckillRetBO> seckill(@PathVariable String pathId, @Valid @RequestBody SeckillQuery fshopQuery) {
        // 验证path
        boolean isPass = seckillService.verifyPath(ContextHandler.getCurrentUserId(), fshopQuery.getProductId(), pathId);
        if(!isPass){
            return failure(GlobalExCode.ILLEGAL_OPERATE);
        }
        // 执行秒杀业务
        return success(seckillService.seckill(fshopQuery));
    }
    ...
    public GlobalRet<SeckillRetBO> getSeckillRet(@RequestParam long productId) {
        return success(seckillService.getSeckillRet(ContextHandler.getCurrentUserId(), productId));
    }
